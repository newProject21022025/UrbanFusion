...
export default function UserOrders() {
  ...
  const currentLocale = (params.locale as "en" | "uk") || "uk";
  ...
  // Форматування ціни як в Favorites:
 const formatPrice = (amount: number) => {
  ...
  // Нормалізація / мапінг статусів — доповніть, якщо бекенд повертає інші значення
  const getStatusKey = (status: string | undefined) => {
    ...
    const map: Record<string, string> = {
      ...
      shipped: "shipped",
      canceled: "canceled",
      cancelled: "canceled", // англ. alternate spelling
      // якщо бекенд повертає інші варіанти, додайте їх сюди
    };
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("failedToFetchOrders"));
      ...
      console.error(t("failedToFetchOrders"), err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={styles.container}>
      ...
      {orders.map((order) => {
        ...
        const orderTotal = order.items
          .reduce(
            (total, item) =>
              total + item.quantity * item.price.amount * (1 - item.price.discount / 100),
            0
          );
        ...
        return (
          <div key={order._id} className={styles.orderCard}>
                ...
                <div className={styles.imageBox} key={i}>
                  {item.mainImage?.url && (
                    <img
                      ...
                      alt={item.mainImage.alt ? item.mainImage.alt[currentLocale] || "" : ""}
                    />
                  )}
                </div>
              ))}
            ...
            <div className={styles.orderBox}>
              ...
              <ul className={styles.orderItems}>
                {order.items.map((item, i) => (
                  <li key={i} className={styles.noBullet}>
                    <ul>
                      <li>
                        {t("itemName")}: {item.name[currentLocale]}
                      </li>
                      <li>
                      ...
                      <li>
                        ...
                        <span style={{ fontWeight: "bold" }}>
                          {formatPrice(item.price.amount * (1 - item.price.discount / 100))}
                        </span>
                      </li>
                      <li>
                        {t("totalAmount")}:{" "}
              ...
              <p>
                <strong>{t("orderTotal")}:</strong> {formatPrice(orderTotal)}
              </p>
            </div>
          </div>
        );
